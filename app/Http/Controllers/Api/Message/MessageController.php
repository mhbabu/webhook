...
class MessageController extends Controller
{
    ...
    public function incomingMsg(Request $request)
    {
        ...
        $InteractionType = $data['messageData']['InteractionType'] ?? null;
        ...
        // ----------------------------
        // 2. FACEBOOK / INSTAGRAM FLOW
        // ----------------------------
        if (in_array($source, ['facebook', 'instagram'])) {

            DB::transaction(function () use ($agentId, $availableScope, $conversationId, $InteractionType, $source) {
                ...
                if ($InteractionType === 'new' && empty($conversation->agent_id)) {
                    $conversation->agent_id = $user->id;
                    $conversation->agent_assigned_at = now();
                    $conversation->save();
                }

                // Optional (if really needed later)
                ...
                DB::afterCommit(function () use ($conversation, $user, $source) {
                    ...
                    $channelData = ['platform' => $source, 'agentId' => $user->id];
        ...
        DB::transaction(function () use (
            ...
            if ($InteractionType === 'new' || empty($message->receiver_id)) {
            ...
            DB::afterCommit(function () use (
                ...
                // Fire system message only once
                if ($isFirstAssignment && $source === 'whatsapp') {
                    event(new SendSystemConfigureMessageEvent(
                        $conversation,
                        $user,
                        $message->id,
                        'agent_assigned'
                ...
                broadcast(new SocketIncomingMessage($payload, [
                    'platform' => $source,
                    'agentId' => $user->id,
                ]));
            });
    ...
    protected function sendEmailMessageFromAgent(array $data, array $attachments, Conversation $conversation, Customer $customer)
    {
        ...
        $conversation->update(['end_at' => now(), 'wrap_up_id' => '30', 'ended_by' => $authId, 'first_response_at' => $conversation->first_response_at ?? now()]); // Auto-end email conversations
        ...
        Log::info('Email message sent from agent', ['user' => $user, 'conversation' => $conversation]);
