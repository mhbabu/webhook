...
class FacebookAdapter
{
    ...
    public function syncPosts(Platform $platform, PlatformAccount $account)
    {
        ...
        $token = $this->resolvePageToken($platform);
        ...
        Log::info('Facebook posts sync response: ', $res->json());
    ...
    protected function resolvePageToken(Platform $platform): string
    {
        $page = PlatformAccount::where('platform_id', $platform->id)->first();

        if (! $page) {
            Log::error('Facebook Page account not found', [
                'platform_id' => $platform->id,
            ]);
            throw new \RuntimeException('Facebook Page account not found');
        }

        $token = $page->credentials['page_token'] ?? null;

        if (! $token) {
            Log::error('Facebook Page access token missing', [
                'platform_account_id' => $page->id,
            ]);
            throw new \RuntimeException('Facebook Page access token missing');
        }
